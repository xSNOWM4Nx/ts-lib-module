...
export abstract class Service implements IService {
  ...
  protected resolveNotRunningResponse = <T>(action: string, caller: string, payload?: T) => {

    var displayKey = "service.notrunning";
    var displayValue = `Service '${this.key}' is not running.`;
    var logMessage = `${displayValue} '${caller}' cannot perform action '${action}'.`;
  ...
  protected resolveNoValidObjectResponse = <T>(caller: string, payload?: T) => {

    var displayKey = "service.novalidobject";
    var displayValue = `Service '${this.key}' received a non valid object.`;
    var logMessage = `${displayValue} '${caller}' has received an object that is null or undefined.`;
    this.logger.error(logMessage);
